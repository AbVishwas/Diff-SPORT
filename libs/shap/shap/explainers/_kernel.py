...
class KernelExplainer(Explainer):
    ...
    def explain(self, incoming_instance, **kwargs):
        # Convert incoming input to a standardized SHAP instance
        instance = convert_to_instance(incoming_instance)
        match_instance_to_data(instance, self.data)

        # Find which features vary from the background dataset
        ...
        # Evaluate the model on the full instance (all features present)
        if self.keep_index:
        ...
        # If no features vary, return zero importance
        if self.M == 0:
            phi = np.zeros((self.data.groups_size, self.D))
            phi_var = np.zeros((self.data.groups_size, self.D))
            return phi

        # If only one feature varies, assign it the full difference
        elif self.M == 1:
            phi = np.zeros((self.data.groups_size, self.D))
            phi_var = np.zeros((self.data.groups_size, self.D))
        ...
        # Define the min and max subset size range for medium-sized coalitions
        min_size = max(1, int(0.1 * self.M))  # At least 1 feature
        max_size = min(self.M - 1, int(0.4 * self.M))  # At most M-1 features
        print(f"min:{min_size}, max:{max_size}")

        # Determine valid subset sizes that fall within the medium range
        valid_sizes = np.arange(min_size, max_size + 1)

        # Determine the number of samples if not provided
        ...
        # If M is small, enumerate all feature subsets
        ...
        weight_vector = np.zeros(self.M - 1)  # Only considering subset sizes from 1 to M-1

        # Assign nonzero weight only to medium-sized coalitions
        for size in valid_sizes:
            weight_vector[size - 1] = (self.M - 1.0) / (size * (self.M - size))

        # Normalize weights to ensure they sum to 1 (avoid division by zero)
        if np.sum(weight_vector) > 0:
            weight_vector /= np.sum(weight_vector)

        # Compute subset size-related variables based on medium-sized coalitions only
        num_subset_sizes = len(valid_sizes)  # Number of valid subset sizes in medium range
        num_paired_subset_sizes = num_subset_sizes // 2  # Half of them can be paired

        log.debug(f"Valid subset sizes: {valid_sizes}")
        log.debug(f"Weight vector after modification: {weight_vector}")
        log.debug(f"Number of medium-sized subsets: {num_subset_sizes}")
        log.debug(f"Number of paired medium-sized subsets: {num_paired_subset_sizes}")

        # Enumerate and sample coalitions
        num_full_subsets = 0
        ...
        for subset_size in valid_sizes:  # Loop only through medium-sized subsets
            nsubsets = binom(self.M, subset_size)
            if subset_size <= num_paired_subset_sizes:
                nsubsets *= 2  # Pairing subsets

            # If we have enough samples, fully enumerate this subset size
            if num_samples_left * remaining_weight_vector[subset_size - 1] / nsubsets >= 1.0 - 1e-8:
                num_full_subsets += 1
                num_samples_left -= nsubsets

        # Randomly sample from the remaining subset space (only medium-sized subsets)
        if num_full_subsets != num_subset_sizes:
            ind_set = np.random.choice(valid_sizes, 4 * num_samples_left, p=weight_vector[min_size - 1:max_size])
            ind_set_pos = 0
            used_masks = {}
            while num_samples_left > 0 and ind_set_pos < len(ind_set):
                mask.fill(0.0)
                subset_size = ind_set[ind_set_pos]
                ind_set_pos += 1
        ...
        # Run the model on the generated coalitions
        self.run()

        # Solve for feature importance (Shapley values)
        phi = np.zeros((self.data.groups_size, self.D))
        phi_var = np.zeros((self.data.groups_size, self.D))
        for d in range(self.D):
