#adapted from https://github.com/bahjat-kawar/ddrm/blob/master/functions/svd_replacement.py
import torch
...
class Inpainting_custom(H_functions):
    ...
    def add_zeros(self, vec):
        raise NotImplementedError
        # No need to implement this since the dimensionality doesn't change
        #return vec

    def singulars(self):
        return self._singulars.float()

    def H(self, vec): #We are multiplying the mask to all the images
        return vec.reshape(vec.shape[0], -1) * self.singulars().reshape((1,-1))
...
class Inpainting_custom_mask_batches(H_functions):
    #def __init__(self, channels, img_dim, missing_indices, device):
        #self.channels = channels
        #self.img_dim = img_dim
        #self.missing_indices = missing_indices
    ...
    def __init__(self, mask_tensor, device):
        self.device = device
        self.mask_tensor = mask_tensor.to(device)
        self._singulars = self.mask_tensor.reshape((mask_tensor.size(0), -1))  # Batch-aware singulars
    ...
    def H(self, vec): #We are multiplying the mask to all the images
        return vec.reshape(vec.shape[0], -1) * self.singulars()
